...
const name = intro ? 'intro' : 'welcome';
const output = new URL(`public/video/mia-${name}.mp4`, root);
try {
  await access(output);
  console.log(`${name} video already exists; no new generation requested.`);
  process.exit(0);
} catch {}
await mkdir(new URL('work/', root), { recursive: true });
await mkdir(new URL('public/video/', root), { recursive: true });
const jobPath = new URL(`work/fal-${name}.json`, root);
const model = 'fal-ai/kling-video/v2.5-turbo/standard/image-to-video';
const prompt = intro
  ? 'Create a gentle ten-second opening movie for Mia’s magical space arcade from this exact toy illustration. Mia waves hello, baby Dean smiles and blinks, and Johnny the chestnut and white spaniel tilts his head. Tiny stars slowly sparkle around their lavender crescent moon. Begin with the original composition, slowly pull back a little to reveal the starry purple sky, and settle into a calm family tableau. Preserve the exact three characters, faces, clothes, dog markings and charming polished toy style. Warm, sweet and welcoming, no dialogue, no text, no cuts, no additional characters or objects, no flashing lights.'
...
if (!job) {
  ...
  console.log(
    `Submitted one ${intro ? 'ten' : 'five'}-second ${name} animation to fal.ai.`,
  );
}
...
while (Date.now() - start < 30 * 60 * 1000) {
  ...
  if (status.status === 'COMPLETED') {
    ...
    console.log(`Saved public/video/mia-${name}.mp4 (${bytes.length} bytes).`);
    process.exit(0);
  }
